						...
						<div class="row">
							<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
								<div class="d-none">
	                            	...
	                            	<tfap:tfapstatuslabel status="APP"/> <!-- A -->
	                                <tfap:tfapstatuslabel status="PAU"/> <!-- P -->
	                                <tfap:tfapstatuslabel status="REJ"/> <!-- R -->
...
<%-- 								<%@ include file="/WEB-INF/views/common/tfap_common_html_datatable.jspf" %> --%>
		                        ...
		                        var logginUsrRoles = "${loggedUsr.getRoleList()}" ; 
									...
									var aoColumnsView${tableName} = [
										...
										},
										{
											...
											"mRender" : function (data, type, full, row) {
												...
												if (status == "RCV" || status == "SUB") { //P // TODO: Need to discuss states
											 		if(reqType == "Link"){
											 			//items += actionItemApprove;
											 			//items += actionItemReject;
											 		}else{
												 		//data = actionsDropdownButtonDisable
														//return data;
											 		}
												}
												else if(status == "APP"  && !(logginUsrRoles.includes('TFAP_BANK_SUP_ADMIN') || logginUsrRoles.includes('TFAP_BANK_ADMIN'))){ //A     //TFAPRW-2338
												 items += actionItemUnsubscribe;
												}
												else if(status=="REJ"){ //R
													//data = actionsDropdownButtonDisable
													//return data;
												}
									...
									<table id="${tableName}"
										...
										<thead>
											<tr class="table_header">
												<th class="column-title"><spring:message code="label.table.approval.TfapId" /></th>
												<th class="column-title"><spring:message code="label.table.approval.Product" /></th>
												<th class="column-title"><spring:message code="label.table.approval.request.type" /></th>
												<th class="column-title"><spring:message code="label.table.approval.BankName" /></th>
												<th class="column-title"><spring:message code="label.table.approval.DtLupd" /></th>
												<th class="column-title"><spring:message code="label.table.approval.Status" /></th>
												<th class="column-title"><spring:message code="label.table.approval.Action" /></th>
											</tr>
										</thead>
										<tbody></tbody>
										<tfoot>
											<tr>
												 <th>
												 	<input type="text" id="search_0"  class="form-control input-xs table-filter inputFilter" placeholder="<spring:message code="label.table.approval.TfapId" />" style="width:100%">
												 </th>
												 <th>
												...
												<th>
													<input type="text" id="search_2" class="form-control input-xs table-filter inputFilter" placeholder="<spring:message code="label.table.approval.request.type" />"style="width:100%">
												</th>
												<th>
													<input type="text" id="search_3" class="form-control input-xs table-filter inputFilter" placeholder="<spring:message code="label.table.approval.BankName" />"style="width:100%">
												</th>
												<th>
													<input type="text" id="search_4" class="form-control input-xs table-filter dateFilter" placeholder="<spring:message code="label.table.approval.DtLupd" />"style="width:100%; background: white" readonly>
												...
												<th>
													<select class="form-control table-filter-status-dropdown"style="width:100%" id="search_5">
														<option value="">All</option>
...
<tfap:tfapconfirmationmodal
	...
	modal_Id="modal_confirm_company_unsubscribe"
	primary_btn_id="btn_confirm_company_unsubscribe"
	message="label.modal.message.confirmation.unsubscribe"
	callback="company_unsubscribeCallback()"/>
	
<tfap:tfapconfirmationmodal
	button_name="label.modal.button.yes"
	modal_title="label.modal.title.confirmation" 
	modal_Id="modal_confirm_company_approve"
	primary_btn_id="btn_confirm_company_approve"
	message="label.modal.message.confirmation.approve"
	callback="company_approveCallback()"/>
	
<tfap:tfapconfirmationmodal
	button_name="label.modal.button.yes"
	modal_title="label.modal.title.confirmation" 
	modal_Id="modal_confirm_company_reject"
	primary_btn_id="btn_confirm_company_reject"
	message="label.modal.message.confirmation.reject"
	callback="company_rejectCallback()"/>
	
<tfap:modal button_name="label.modal.button.continue"
	modal_title="label.modal.title.success"
	modal_Id="modal_company_unsubscribe_request" show_button_primary="false">
	<div class="row">
		<h4 style="text-align: center;"><spring:message code="modal.company.product.unsubscription.message"/></h4>
	</div>
</tfap:modal>
...
$(document).ready(function(){
	...
	Number.prototype.pad = function(size) {
	 ...
	 $('#${tableName}').on('click', '.unsubscribe', function(){
			...
			$('#modal_confirm_company_unsubscribe').modal('show');	
	});
	 
	$('#${tableName}').on('click', '.approve', function(){
		var row=$("#${tableName}").DataTable().row($(this).parents('tr')).data();
		det_tfpslAccnid = row.tfpslAccnid;
		det_tfpslAccnidBnk = row.tfpslAccnidBnk;
		det_tfpslPrdId = row.tfpslPrdId;
		det_tfpslTfapId = row.tfpslTfapId;
		$('#modal_confirm_company_approve').modal('show');
	});
	 
	 $('#${tableName}').on('click', '.reject', function(){
		var row=$("#${tableName}").DataTable().row($(this).parents('tr')).data();
		det_tfpslAccnid = row.tfpslAccnid;
		det_tfpslAccnidBnk = row.tfpslAccnidBnk;
		det_tfpslPrdId = row.tfpslPrdId;
		det_tfpslTfapId = row.tfpslTfapId;
		$('#modal_confirm_company_reject').modal('show');			
	});
...
function company_unsubscribeCallback(){
	...
	$.get("/TFAPortal/tfApplication/product-management/prd-approval-list-table/unsubscribe?tfpslAccnid="+det_tfpslAccnid+"&tfpslAccnidBnk="+ det_tfpslAccnidBnk+"&tfpslPrdId="+det_tfpslPrdId+"&tfpslTfapId="+det_tfpslTfapId+'&fromCom=true', (res) => {
		const resJson = JSON.parse(res);
		if (resJson.unsubscribecom) {
			swal("", '<spring:message code="modal.company.product.unsubscription.message"/>', "success").then(() => {
				$("#${tableName}").DataTable().draw();
			});
		}
		else {
			swal("", '<spring:message code="generic.error.message"/>', "error");
		}
	}).fail(() => {
		swal("", '<spring:message code="generic.error.message"/>', "error");
	});
	// $('#modal_company_unsubscribe_request').modal('show');
}

function company_approveCallback(){
...
function company_rejectCallback(){
