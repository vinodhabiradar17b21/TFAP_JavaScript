...
<tfap:modal button_name="label.modal.button.continue"
	...
	<div class="row px-2">
		<h4 style="text-align: center;">You have declined and returned this application to the bank maker.</h4>
	</div>
</tfap:modal>
	...
	$.ajax({
	    ...
	    success: function( data, textStatus, jQxhr ){
	    	...
	    	//	$('#modals_bank_reject').modal('show');	
	    	  swal('<spring:message code="label.modal.message.success.decline"/>', "","success").then(function() {
	    		  if(dataTable != ''){
	    				dataTable.draw();
	    			}else{
	    				let redirectUrlDecline = redirectUrl;
	    				if (${(not empty modelNameUrl)}) {
	    					redirectUrlDecline = redirectUrl.replace("tfApplication", '${modelNameUrl}');
	    				}
	    		  		window.location = redirectUrlDecline;
	    			}
            	});
