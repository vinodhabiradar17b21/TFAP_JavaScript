...
<tfap:modal button_name="label.modal.button.continue"
	...
	<div class="row px-2">
		<h4 style="text-align: center;">The application has been initiated for rejection.</h4>
	</div>
</tfap:modal>
	    	...
	    	else
	    		{
	    		 if(data.reject){
	 	        	...
	 	        	$('#modal_confirm_initiate_reject').modal('hide');
	 	        	  swal( "The application has been initiated for rejection.", "","success").then(function() {
	 	        			if(dataTable != ''){
	 	        				dataTable.draw();
	 	        			}else{
	 	        				let redirectUrlInitRej = redirectUrl;
	 	        				if (${(not empty modelNameUrl)}) {
	 	        					redirectUrlInitRej = redirectUrl.replace('tfApplication', '${modelNameUrl}');
	 	        				}
	 	        			    window.location = redirectUrlInitRej;
	 	        			}
	 	              });
