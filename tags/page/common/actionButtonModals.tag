...
<%@attribute name="tranId" type="java.lang.String" required="false"%>
<%@attribute name="status" type="java.lang.String" required="true"%>
<%@attribute name="modalType" type="java.lang.String" required="false" description="Type of modal to show, e.g rejection etc."%>
<%@attribute name="appRootUrl" type="java.lang.String" required="false" description="For redirection"%>



<c:if test="${fn:contains('NEW,DRF', pageScope.status) }">
...
<c:if test="${pageScope.modalType eq 'VERIFICATION' }">
...
<c:if test="${pageScope.modalType eq 'MAKER_APPROVER' }">
	<!-- Modal for the approver action for approved application -->
	<tfap:modal button_name="label.bank.button" modal_title="label.bank.maker.approver.check" modal_Id="modal_maker_approver_id"
				show_button_close="false" show_button_primary="false">
				<br />
		<div class="modal-content">
			<div class="modal-body">
			...
			<div class="modal-footer" style="margin-top: -20px;">
				<button type="button" class="btn btn-outline-primary mr-1"
					...
					href="${ctx}/tfApplication/list/view"
					data-dismiss="modal">
					<spring:message code='label.modal.button.close' />
				</button>
			</div>
...
<%--<c:if test="${pageScope.modalType eq 'MAKER_VERIFIER' }">--%>
	<tfap:modal button_name="label.bank.button"
...
<c:if test="${pageScope.modalType eq 'REJECTION_REASON' && fn:contains('PAU', pageScope.status)}">
	<!-- Button submit -->
...
<c:if test="${pageScope.modalType eq 'REJECTION' && fn:contains('PAU', pageScope.status)}">
	<!-- Modal for Rejection -->
...
<c:if test="${pageScope.modalType eq 'DISCARD' }">
	...
	<tfap:tfapconfirmationmodal button_name="label.modal.button.yes"
		...
		callback="discardCallback('${pageScope.appRootUrl },${pageScope.tranId }', '${pageScope.msgType }')" />
