...
<c:set var="disableBnkSpecificSelect"
       value="${(empty disableBnkSpecificSelect) ?
	((empty isDisabled)?((empty isdisabled)?false:isdisabled):isDisabled) : disableBnkSpecificSelect}"/>
...
<c:choose>
	...
	<div class="btn-group ml-1 submitter-group">
			...
			<c:choose>
				<c:when test="${(tfApplication.tfTransaction.tfTran.msgType == 'IMPST' || tfApplication.tfTransaction.tfTran.msgType == 'ISDEB' || tfApplication.tfTransaction.tfTran.msgType == 'ISILN' || tfApplication.tfTransaction.tfTran.msgType == 'ISOTH') && tfApplication.importsettlement.tfImpSettlement.isBillCreatedByCompany!='YES'}">
					<div class="input-group-text">
					<c:out value="${tfApplication.tfTransaction.tfTran.tftBnkName}" />
					</div>
				</c:when>
				<c:otherwise>
					<spring:message code='label.selectbank' var="selectfieldLabel" />
					<form:select id="selectedBank1"
						path="tfApplication.tfTransaction.tfTran.tftBnkAccn"
						cssErrorClass="border-danger form-control required"
						cssClass="form-control required" required="true" disabled="true">
			...
			<div class="modal-content">
				...
				<div class="modal-footer" style="margin-top: -20px;">
					<button type="button" class="btn btn-outline-primary mr-1"
						...
						href="/TFAPortal/tfApplication/list/view"
						data-dismiss="modal">
...
<tfap:tfapconfirmationmodal
		button_name="label.modal.button.yes"
		modal_title="label.modal.title.confirmation" 
		modal_Id="modal_confirm_submit_for_auth_settlement_amount_different"
		primary_btn_id="btn_confirm_submit_for_auth"
		message="label.modal.message.confirmation.submit.for.auth.amount.different" 
		callback="submitForAuthCallback()"/>
		
<tfap:tfapconfirmationmodal
		button_name="label.modal.button.yes"
		modal_title="label.modal.title.confirmation" 
		modal_Id="modal_confirm_submit_for_auth_settlement_currency_different"
		primary_btn_id="btn_confirm_submit_for_auth"
		message="label.modal.message.confirmation.submit.for.auth.currency.different" 
		callback="submitForAuthCallback()"/>

<div class="modal hide" id="pleaseWaitDialog" data-backdrop="static"
	data-keyboard="false" role="dialog" style="display: none;">
	<div class="modal-dialog modal-lg">
		<!-- Modal content-->
		<div class="modal-bodyLoader text-center" style="margin-top:150px"">
...
function redirectToMainSub(e){
	window.location.href="/TFAPortal/tfApplication/list/view";
}
    function submit_form(action) {
        ...
        if (action == 'submit') {
    		...
    		if(makerComApproverCheck === 'RESTRICT'){
    			...
    			// Validate the record open/close status:
                draftSubmitFunction(tfapId, appRefType, rootUrl);
        		
    			/* $('#editForm').attr('action', rootUrl + '/draft/submit?doc_type=${tfApplication.tfTransaction.tfTran.msgType}');
    		...
    		if(makerComApproverCheck === 'RESTRICT'){
    			...
    			// Validate the record open/close status:	
    			 draftSaveFunction(tfapId, appRefType, rootUrl);
        		
    			/* $('#editForm').attr('action', rootUrl + '/draft/save?doc_type=${tfApplication.tfTransaction.tfTran.msgType}');
    			$('#editForm').submit(); */
    		}
    ...
    $("#selectedBank").on('change', function () {
        $('#submit_review_btn').prop('disabled', $('#selectedBank :selected').val() == "");
 ...
 function draftSubmitFunction(tfapId, appRefType, rootUrl){
	var docType = '${tfApplication.tfTransaction.tfTran.msgType}';
			...
			success: function(result){
				...
				if(isRecordOpen!=null && isRecordOpen == 'true'){
					...
					.then(function(willDelete) {
					  if (willDelete) {
						$('#editForm').attr('action', rootUrl + '/draft/submit?doc_type='+docType);
						if(docType == 'ISDEB' || docType == 'ISILN' || docType == 'ISOTH' || docType == 'IMPST') {
			    			/* $('#modal_confirm_submit_for_auth_settlement').modal('show');  */
			    			showHideAuthSettlementModal(rootUrl, docType, tfapId);
						} else {
			    			$('#modal_confirm_submit_for_auth').modal('show'); 
						}
					  }
 ...
 function draftSaveFunction(tfapId, appRefType, rootUrl){
	...
	function showHideAuthSettlementModal(rootUrl, docType, tfapId) {
		$.ajax({
			url: rootUrl+"/draft/validateCcyAmt?doc_type="+docType+"&tfapId="+tfapId,
			type: "POST",
			data: $("#editForm").serialize(),
			success: function(result) {
				const res = JSON.parse(result);
				var isCcyEqual = res.isCcyEqual;
				var isAmtEqual = res.isAmtEqual;

				if (isCcyEqual != undefined || isCcyEqual != null) {
					if (!isCcyEqual) {
						$('#modal_confirm_submit_for_auth_settlement_currency_different').modal('show'); 
					}
				}

				if (isAmtEqual != undefined || isAmtEqual != null) {
					if (!isAmtEqual) {
						$('#modal_confirm_submit_for_auth_settlement_amount_different').modal('show'); 
					}
				}

				if (isCcyEqual && isAmtEqual) {
					submitForAuthCallback();
				}
			}
		});
